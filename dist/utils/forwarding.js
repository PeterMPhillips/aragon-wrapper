...
function parseForwardCall(calldata) {
  ...
  const offset = parseInt("0x".concat(evmscriptData.substring(0, 64)), 16) * 2; // The first word in the data is its length (uint256); actual data starts after

  const startIndex = offset + 64; // Parse length of the data stored
  // (it's stored as bytes so we need to parse in hex first and then multiply by 2)

  const dataLength = parseInt("0x".concat(evmscriptData.substring(offset, startIndex)), 16) * 2; // Grab the data stored in the bytes array
