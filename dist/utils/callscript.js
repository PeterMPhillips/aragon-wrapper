...
function decodeSegment(script) {
  // Get address
  const to = "0x".concat(script.substring(0, 40));
  script = script.substring(40); // Get data

  const dataLength = parseInt("0x".concat(script.substring(0, 8)), 16) * 2;
  script = script.substring(8);
  const data = "0x".concat(script.substring(0, dataLength)); // Return rest of script for processing
