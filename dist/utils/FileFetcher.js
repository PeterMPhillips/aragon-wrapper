...
class FileFetcher {
  ...
  getFullPath(provider, location, path) {
    ...
    const baseLocation = provider === 'ipfs' ? "".concat(this.providers.get('ipfs').gateway).concat(location) : location;
    return "".concat(sanitizeUrl(baseLocation)).concat(sanitizePath(path));
  }

  async fetch(provider, location, path, config) {
    const response = await axios(this.getFullPath(provider, location, path), _objectSpread({}, config, {
      responseType: 'text',
