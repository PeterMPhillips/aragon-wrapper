...
class Aragon {
  constructor(daoAddress, options = {}) {
    ...
    this.datastore = new _datastore.default(options.datastore);
  }
  /**
   ...
   * @param {Object} [options.datastore] `initApps()` options (see below)
   * @return {Promise<void>}
    ...
    const fetchedPermissions$ = (0, _rxjs.concat)(pastEvents$, currentEvents$).pipe((0, _operators.scan)(([permissions], event) => {
      ...
      if (eventData.app) {
        ...
        if (event.event === SET_PERMISSION_EVENT) {
          const key = "".concat(eventData.role, ".allowedEntities"); // Converts to and from a set to avoid duplicated entities
        ...
        if (event.event === CHANGE_PERMISSION_MANAGER_EVENT) {
          ...
          _dotProp.default.set(appPermissions, "".concat(eventData.role, ".manager"), eventData.manager);
        }
  ...
  initApps() {
    ...
    (0, _operators.withLatestFrom)(installedApps$, function updateApps(setAppEvent, apps) {
      ...
      return apps.map(async app => {
        ...
        return _objectSpread({}, app, {}, proxyValues, {
          updated: true
        });
      });
    ...
    const appsWithInfo$ = apps$.pipe((0, _operators.concatMap)(apps => Promise.all(apps.map(async app => {
      ...
      if (app.appId && app.codeAddress) {
        const cacheKey = "".concat(app.appId, ".").concat(app.codeAddress);
      ...
      return _objectSpread({}, appInfo, {}, app, {}, isForwarderOverride);
    }))));
    ...
    (0, _operators.concatMap)(async ([newRepoAppIds, updatedRepoAppIds]) => {
      const newRepos = (await Promise.all(newRepoAppIds.map(async appId => {
        ...
        try {
          ...
          console.error("Could not find repo for ".concat(appId), err);
        }
    ...
    (0, _operators.mergeMap)(([newRepos, updatedRepoAppIds]) => {
      ...
      (0, _operators.mergeMap)(async ({
        ...
        (0, _operators.concatMap)(async event => {
          ...
          return _objectSpread({}, versionInfo, {
            timestamp
          });
    ...
    (0, _operators.scan)(({
      ...
      const updatedRepoInfo = _objectSpread({}, repos[updatedAppId], {}, update);
    ...
    (0, _operators.concatMap)(async ({
      ...
      const fetchVersionInfo = version => {
        return applicationInfoCache.request("".concat(appId, ".").concat(version.contractAddress)).catch(() => ({})).then(content => ({
          content,
      ...
      if (!currentVersion) {
        ...
        return _objectSpread({}, baseRepoInfo, {
          currentVersion: null,
  ...
  async requestAppPath(appAddress, path) {
    ...
    if (!(await this.getApp(appAddress))) {
      throw new Error("Cannot request path for non-installed app: ".concat(appAddress));
    }
  ...
  async runApp(proxyAddress) {
    ...
    return sandboxMessengerProvider => {
      ...
      handlers.createRequestHandler(request$, 'identify', handlers.appIdentifier), handlers.createRequestHandler(request$, 'address_identity', handlers.addressIdentity), handlers.createRequestHandler(request$, 'search_identities', handlers.searchIdentities), // Datastore handlers
      handlers.createRequestHandler(request$, 'datastore', handlers.datastore)).subscribe(response => messenger.sendResponse(response.id, response.payload)); // The attached unsubscribe isn't automatically bound to the subscription
  ...
  async getPermissionManager(appAddress, roleHash) {
    ...
    return _dotProp.default.get(appPermissions, "".concat(roleHash, ".manager"));
  }
  /**
  ...
  async getACLTransactionPath(method, params) {
    ...
    if (!functionArtifact) {
      throw new Error("Method ".concat(method, " not found on ACL artifact"));
    }
    ...
    if (functionArtifact.roles && functionArtifact.roles.length !== 0) {
      ...
      if (!methodABI) {
        throw new Error("Method ".concat(method, " not found on ACL ABI"));
      }
      ...
      if (appIndex === -1 || roleIndex === -1) {
        throw new Error("Method ".concat(method, " doesn't take _app and _role as input. Permission manager cannot be found."));
      }
  ...
  async applyTransactionGas(transaction, isForwarding = false) {
    ...
    const estimatedGasLimit = await this.web3.eth.estimateGas(_objectSpread({}, transaction, {
      gas: undefined
    }));
  ...
  async calculateTransactionPath(sender, destination, methodSignature, params, finalForwarder) {
    ...
    if (finalForwarderProvided) {
      ...
      const allowedEntities = _dotProp.default.get(destinationPermissions, "".concat(roleSig, ".allowedEntities"), []); // No one has access, so of course we don't as well
